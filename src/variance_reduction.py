...
def antithetic_variates(
    ...
    z_As  = np.random.normal(0, 1, m)
    z_ls  = np.random.normal(0, 1, m)
    z_Af  = np.random.normal(0, 1, m)
...
def importance_sampling(
    ...
    z_As = np.random.normal( shift,          1, n)
    z_ls = np.random.normal(-shift * 0.3,    1, n)
    z_Af = np.random.normal( shift * 0.5,    1, n)
...
def _naive_wrap(n, **kw):
    from simulation import naive_mc_dose
    return naive_mc_dose(n, **kw)
...
def benchmark_vrf(
    ...
    baseline_estimates = [naive_mc_dose(n, organ)[0] for _ in range(n_trials)]
    baseline_var = np.var(baseline_estimates)

    vrfs = {}
    for name, fn in ALL_METHODS.items():
        estimates = [fn(n, organ=organ)[0] for _ in range(n_trials)]
        v = np.var(estimates)
